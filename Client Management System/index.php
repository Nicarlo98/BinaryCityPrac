<?php
// Database connection
$pdo = new PDO('mysql:host=localhost;dbname=client_contact_db', 'username', 'password');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (isset($_POST['create_contact'])) {
        $name = $_POST['name'];
...
<head>
    ...
    <style>
        ...
        input[type="text"], input[type="email"] { width: 100%; padding: 5px; margin: 5px 0; }
...
<body>
    ...
    <form method="post">
        ...
        <select name="contact_id">
            <?php foreach ($contacts as $contact): ?>
                <option value="<?= $contact['id'] ?>"><?= htmlspecialchars($contact['surname'] . ', ' . $contact['name']) ?></option>
            <?php endforeach; ?>
        </select>
